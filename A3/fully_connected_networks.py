...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        flattened_x=x.view(N,D)
        out=torch.mm(flattened_x,w)+b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        H=hidden_dim
        C=num_classes
        self.params["W1"]=torch.normal(mean=0.0,std=weight_scale,size=[N,H],dtype=dtype)
        self.params["b1"]=torch.zeros(size=[H],dtype=dtype)
        self.params["W2"]=torch.normal(mean=0.0,std=weight_scale,size=[H,C],dtype=dtype)
        self.params["b2"]=torch.zeros(size=[C],dtype=dtype)
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        LinearLayer1_out,LinearLayer1_cache=Linear.forward(X,self.params["W1"],self.params["b1"])
        ReLU1_out,ReLU1_cache=ReLU.forward(LinearLayer1_out)
        LinearLayer2_out,LinearLayer2_cache=Linear.forward(ReLU1_out,self.params["W2"],self.params["b2"])
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
          ...
          self.params[name]=torch.normal(0,weight_scale,size,dtype=dtype)
          self.params["b"+str(i)]=torch.zeros(size[1],dtype=dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver=Solver(model,data_dict,optim_config=config,update_rule=rmsprop)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def sgd_momentum(w, dw, config=None):
    ...
    ##################################################################
    # Replace "pass" statement with your code
    v=dw*(1-config["momentum"])+config["momentum"]*v
...
def rmsprop(w, dw, config=None):
    ...
    cache=config["cache"]
    cache=decay_rate*dw+(1-decay_rate)*cache**2
    next_w=w-cache/math.sqrt(torch.sum(cache**2)+epsilon)*learning_rate
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    next_w=w-learning_rate*moment1_unbias/(moment2_unbias.sqrt()+epsilon)
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
